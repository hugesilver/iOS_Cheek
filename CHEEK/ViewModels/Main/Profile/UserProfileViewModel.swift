...
class UserProfileViewModel: ObservableObject {
    ...
    // 프로필 조회
    func getProfile(targetMemberId: Int64, loginMemberId: Int64) {
        print("프로필 조회 시도")
        
        var components = URLComponents(string: "\(ip)/member/info/\(targetMemberId)")!
        
        components.queryItems = [
            URLQueryItem(name: "loginMemberId", value: "\(loginMemberId)")
        ]
        ...
        CombinePublishers().urlSession(req: request)
            ...
            .sink(receiveCompletion: { completion in
                switch completion {
                case .finished:
                    print("getProfile 함수 실행 중 요청 성공")
                case .failure(let error):
                    print("getProfile 함수 실행 중 요청 실패: \(error)")
                }
